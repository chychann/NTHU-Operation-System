...
AddrSpace::~AddrSpace()
{
   ...
   for(int i=0;i<numPages;i++){
     kernel->releaseframe(pageTable[i].physicalPage);
...
for(int i=0;i<numPages;i++){
  ...
  int find;
  find=kernel->findfreeframe();
  
  if(find!=-1){
  
  bzero(&kernel->machine->mainMemory[find*PageSize], PageSize);
  
  pageTable[i].physicalPage=find;//link
  AddrSpace::NumFreePages --;
  //cout<<i<<"->"<<find<<endl; //list all table valid
