...
class FileSystem {
  public:
	...
	int fileDescriptor = OpenForWrite(name);//OpenForWrite 是在C++內建的函數
  ...
  OpenFileId OpenAFile(char *name) {
      for(int i=0;i<20;i++){
          if(OpenFileTable[i]==NULL){
              ...
              //OpenFile 會設定fileDescriptor (in openfile.h)
              return i;
          }
      }
      return -1;
    ...
    int WriteAFile(char *buffer, int size, OpenFileId id){
        //cout<<"write\n";
        if(size<0 || id<0 ||id>19 ) return -1;
        if(OpenFileTable[id]==NULL) return -1;
        //cout<<"write\n";
        WriteFile(OpenFileTable[id]->file ,buffer,size);
        //cout<<"write ok\n";
        return size;
    }
    ...
    int ReadFile(char *buffer, int size, OpenFileId id){
        ...
        if(size<0 || id<0 || id>19) return -1;
        if(OpenFileTable[id]==NULL) return -1;
        return ReadPartial(OpenFileTable[id]->file,buffer,size);
    }
    ...
    if(OpenFileTable[id]){
      //cout<<"close1\n";
      if(Close(OpenFileTable[id]->file)>=0){
