...
Machine::Run()
{
    ...
    if (debug->IsEnabled('m')) { //ISEnabled return 1 if debug msg with 'flag'
        cout << "Starting program in thread: " << kernel->currentThread->getName();
	cout << ", at time: " << kernel->stats->totalTicks << "\n";
    }
    
    kernel->interrupt->setStatus(UserMode);//set usermode
	...
	if (singleStep && (runUntilTime <= kernel->stats->totalTicks))
		Debugger();// 	Primitive debugger for user programs.
    }
}
